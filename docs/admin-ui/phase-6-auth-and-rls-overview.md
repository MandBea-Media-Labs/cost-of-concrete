...
- Introduce an explicit **admin role** via `user_profiles.is_admin`.
...
- Trigger `user_profiles_updated_at` reuses the shared `update_updated_at_column()` function.
...
Result: RLS now enforces that **only admins** can manage pages via Supabase, while non-admin/public users can only read published content through the existing read-only policy.
