...
const personaFormSchema = z.object({
  ...
  maxTokens: z.coerce.number().int().min(100).max(100000).default(4096),
...
<template>
  <UiSheet :open="open" @update:open="emit('update:open', $event)">
    <UiSheetContent side="right" class="w-full sm:max-w-xl overflow-y-auto">
      <UiSheetHeader>
      ...
      <form class="mt-6 space-y-5" @submit="onSubmit">
        ...
        <!-- Actions -->
        <UiSheetFooter class="mt-6 gap-2">
